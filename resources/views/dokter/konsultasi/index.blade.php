                            ...
                            <div class="flex-grow-1" style="min-width: 0;">
                                ...
                                <div class="d-flex align-items-center gap-2 mb-1">
                                    ...
                                    @if ($msg->from_user->kode_pasien)
                                        <span class="badge bg-info text-white px-2 py-1" style="font-size: 0.7rem;">
                                            <i class="fas fa-id-card me-1"></i>{{ $msg->from_user->kode_pasien }}
                                        </span>
                                    @endif
                                </div>
                                ...
                                @if ($msg->from_user->profile)
                                    <div class="d-flex flex-wrap gap-1 text-muted mb-2" style="font-size: 0.75rem;">
                                        <span>
                                            <i
                                                class="fas fa-{{ $msg->from_user->profile->jenis_kelamin == 'L' ? 'mars text-primary' : 'venus text-danger' }} me-1"></i>
                                            {{ $msg->from_user->profile->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}
                                        </span>
                                        @if ($msg->from_user->profile->tanggal_lahir)
                                            <span>•</span>
                                            <span><i
                                                    class="fas fa-birthday-cake me-1"></i>{{ \Carbon\Carbon::parse($msg->from_user->profile->tanggal_lahir)->age }}
                                                tahun</span>
                                        @endif
                                        @if ($msg->from_user->profile->golongan_darah && $msg->from_user->profile->golongan_darah != '-')
                                            <span>•</span>
                                            <span class="badge bg-danger-subtle text-danger px-1 py-0"
                                                style="font-size: 0.7rem;">
                                                <i
                                                    class="fas fa-tint me-1"></i>{{ $msg->from_user->profile->golongan_darah }}
                                            </span>
                                        @endif
                                    </div>
                                @else
                                ...
                                {{-- Pesan --}}
                                <p class="mb-1 {{ $msg->seen == 0 ? 'fw-semibold text-dark' : 'text-muted' }}"
