        ...
        <div class="col-lg-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('staff.obat.update', $obat->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="aturan_pakai" rows="3" 
                                ...
                                required>{{ old('aturan_pakai', $obat->aturan_pakai) }}</textarea>
                            @error('aturan_pakai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="efek_samping" rows="3" 
                                ...
                                required>{{ old('efek_samping', $obat->efek_samping) }}</textarea>
                            @error('efek_samping')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="status" 
                                        ...
                                        <option value="1" {{ old('status', $obat->status) == 1 ? 'selected' : '' }}>
                                            Aktif
                                        </option>
                                        <option value="0" {{ old('status', $obat->status) == 0 ? 'selected' : '' }}>
                                            Nonaktif
                                        </option>
                                    </select>
                                    @error('status')
                            ...
                            @if ($obat->foto)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $obat->foto) }}" 
                                         class="img-thumbnail" width="150" alt="Foto Obat">
                                </div>
                            @endif
