                ...
                <div class="card-body">
                    <form action="{{ route('dokter.catatan.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            @if(!empty($selectedPasien))
                                <input type="hidden" name="user_id" value="{{ $selectedPasien->id }}">
                                <input type="text"
                                       class="form-control"
                                       value="{{ $selectedPasien->name }} - {{ $selectedPasien->kode_pasien }}"
                                       disabled>
                                <small class="text-muted">Pasien sudah dipilih dan tidak bisa diganti di form ini.</small>
                            @else
                                <select name="user_id" class="form-control @error('user_id') is-invalid @enderror" required>
                                    <option value="">-- Pilih Pasien --</option>
                                    @foreach($pasien as $p)
                                        <option value="{{ $p->id }}"
                                            {{ (old('user_id') ?? $selectedUserId) == $p->id ? 'selected' : '' }}>
                                            {{ $p->name }} - {{ $p->kode_pasien }}
                                        </option>
                                    @endforeach
                                </select>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local"
                                   ...
                                   value="{{ old('tanggal_catatan', now()->format('Y-m-d\\TH:i')) }}"
                                   required>
